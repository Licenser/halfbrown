...
use hashbrown::{
    self,
    hash_map::{self, DefaultHashBuilder, Entry as HashBrownEntry},
};
use std::fmt;
...
impl<'a, K, V> From<HashBrownEntry<'a, K, V, DefaultHashBuilder>> for Entry<'a, K, V> {
    fn from(f: HashBrownEntry<'a, K, V, DefaultHashBuilder>) -> Entry<'a, K, V> {
        match f {
...
pub enum OccupiedEntry<'a, K, V> {
    Map(hash_map::OccupiedEntry<'a, K, V, DefaultHashBuilder>),
}
...
pub enum VacantEntry<'a, K, V> {
    Map(hashbrown::hash_map::VacantEntry<'a, K, V, DefaultHashBuilder>),
}
